...
namespace NetManagement.BLL
{
    public class AdminBLL_Category
    {
        ...
        {
            IEnumerable<Category> data = repository.GetAll().ToList();
            repository.Save();
            return data;
        }
        public IEnumerable<Category> GetAllCTR()
        {
            List<Category> data1 = repository.GetAll().ToList();
            return data1;
        }

        public List<Category> Search(string s, string txt)
        {
            List<Category> data1 = GetAllCTR().ToList();
        ...
        public List<Category> Sort(string txt1, string txt2)
        {
            ...
            if (txt1 == "Name")
            {
                ...
                {
                    list = GetAllCTR().OrderBy(p => p.CategoryName).ToList();
                }
                else if (txt2 == "Decrease")
                {
                    list = GetAllCTR().OrderByDescending(p => p.CategoryName).ToList();
                }
            }
            if (txt1 == "ID_Category")
            {
                if (txt2 == "Increase" || txt2 == "")
                {
                    list = GetAllCTR().OrderBy(p => p.ID_Category).ToList();
                }
                else if (txt2 == "Decrease")
                {
                    list = GetAllCTR().OrderByDescending(p => p.ID_Category).ToList();
                }
            }
            return list;
